...
use aeroshield_common::channel::{CHANNELS_2_4, CHANNELS_5};
...
pub fn run(
    ...
    // Start GPS background tracking thread (fails silently if gpsd is not running)
    super::gps::start_gps_thread();
...
fn update_ap(
    ...
    match aps.get_mut(&bssid) {
        Some(ap) => {
            ...
            if ap.latitude.is_none() || ap.longitude.is_none() {
                if let Some((lat, lon)) = super::gps::get_current_coordinates() {
                    ap.latitude = Some(lat);
                    ap.longitude = Some(lon);
                }
            }
        }
        None => {
            let gps_coords = super::gps::get_current_coordinates();
            let latitude = gps_coords.map(|(lat, _)| lat);
            let longitude = gps_coords.map(|(_, lon)| lon);
            aps.insert(
                bssid.clone(),
                AP {
                    essid,
...
/// Derive AeroShield's single-token privacy label from the beacon's security IEs.
fn classify_privacy(capability_info: u16, info: &StationInfo) -> String {
    if let Some(rsn) = &info.rsn_information {
        let has_sae = rsn.akm_suites.contains(&RsnAkmSuite::SAE);
        let has_psk = rsn.akm_suites.contains(&RsnAkmSuite::PSK);
        if has_sae && has_psk {
            return "WPA3/WPA2".to_string();
        } else if has_sae {
