...
use crate::globals::*;
use aeroshield_common::deps::{self, Requirer};
use aeroshield_common::ipc::*;
use aeroshield_common::types::{AP, AttackState, Client};
...
pub fn init() -> Result<(), AgentError> {
    if geteuid().is_root() {
        log::warn!(
            "running the AeroShield GUI as root is discouraged and does not work under Wayland"
        );
    }
    ...
    let mut command = if geteuid().is_root() {
        ...
        if !deps::is_installed(deps::PKEXEC) {
            return Err(AgentError(
                "could not find 'pkexec' to start the privileged agent, install polkit, or run AeroShield as root"
                    .to_string(),
            ));
        }
...
fn agent_binary_path() -> Result<PathBuf, AgentError> {
    ...
    let candidate = exe
        ...
        .join("aeroshield-agent");

    if !candidate.is_file() {
        return Err(AgentError(
            "could not locate the 'aeroshield-agent' binary next to the GUI".to_string(),
        ));
    }
...
pub fn check_update(current_version: &str) -> Option<String> {
    let url = "https://api.github.com/repos/ameramarketing/aeroshield/releases/latest";
    ...
    log::info!("AeroShield is up to date");
